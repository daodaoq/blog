...
const ArticleUpdateForm: React.FC<ArticleUpdateFormProps> = ({ article }) => {
  ...
  const submitForm = async () => {
    ...
    try {
      const response = await axios.put('/api/article/update', request, {
        timeout: 5000,
      });
      ...
      if (response.data.code === 0) {
        console.log('文章更新成功');
        layoutStore.setState({ articleUpdateVisible: false });
      } else {
        console.error('更新失败:', response.data.msg || '未知错误');
      }
    } catch (error) {
      console.error('网络错误，文章更新失败', error);
  ...
  return (
    <div className="article-update-form">
      ...
      <div className="button-group">
        ...
        <button type="button" onClick={() => layoutStore.setState({ articleUpdateVisible: false })}>
          取消
        </button>
      </div>
    </div>
