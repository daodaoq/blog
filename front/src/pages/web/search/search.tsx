...
const ArticleList: React.FC = () => {
  ...
  const downColor = useMemo(() => articleSearchRequest.order === 'desc' ? 'blue' : 'gray', [articleSearchRequest.order]);
  const upColor = useMemo(() => articleSearchRequest.order === 'desc' ? 'gray' : 'blue', [articleSearchRequest.order]);

  const handleSortClick = () => {
    setArticleSearchRequest(prev => ({ ...prev, order: prev.order === 'desc' ? 'asc' : 'desc' }));
  };
  ...
  const getArticleSearchTableData = useCallback(async () => {
    const newRequest = { ...articleSearchRequest, page, page_size: pageSize };
  ...
  return (
    <>
      <WebNavbar />
      <Card className="article-list search-list">
        <Row className="title">文章列表</Row>
        <div className="search">
          <Input
            value={articleSearchRequest.query}
            onChange={(e) => setArticleSearchRequest(prev => ({ ...prev, query: e.target.value }))}
            placeholder="请输入搜索内容"
        ...
        <div className="category">
          ...
          <Radio.Group
            ...
            onChange={(e) => setArticleSearchRequest(prev => ({ ...prev, category: e.target.value }))}
          >
        ...
        <div className="tag">
          ...
          <Radio.Group
            ...
            onChange={(e) => setArticleSearchRequest(prev => ({ ...prev, tag: e.target.value }))}
          >
            <Radio.Button value="">全部</Radio.Button>
            {tagArr.map(item => (
              <Radio.Button key={item} value={item}>{item}</Radio.Button>
        ...
        <div className="sort">
          <Row>排序</Row>
          <Button onClick={() => { handleSortClick(); changeArticleSearchItem(); }}>
            <SortDescendingOutlined style={{ color: downColor }} />
            <SortAscendingOutlined style={{ color: upColor }} />
          </Button>
          <Radio.Group
            value={articleSearchRequest.sort}
            onChange={(e) => setArticleSearchRequest(prev => ({ ...prev, sort: e.target.value }))}
          >
            {sortArr.map(item => (
